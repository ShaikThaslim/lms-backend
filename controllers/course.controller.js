...
export const editCourse=async(req,res)=>{
    try {
        ...
        let newThumbnail=course.courseThumbnail;
        if (uploadedFile){
            ...
            const uploaded = await uploadMedia(uploadedFile.path);
            newThumbnail = uploaded?.secure_url;        }
        

        const updateData={courseTitle,subTitle,description,category,courseLevel,coursePrice,courseThumbnail:newThumbnail}
...
//lecture controller part start from here
...
export const createLecture=async(req,res)=>{
    try {
        ...
        console.error("🔥 Error in createLecture:", error); // << add this
        return res.status(500).json({
            message: "Failed to create lecture",
            error: error.message, // Optional: send the error to frontend
        });
    }
}
...
try {
    ...
    await Course.updateOne( 
        {lectures:lectureId},//find the course and contents from the lecture
